...
**RF06 — busca por texto e filtros por categoria:** implementados no **frontend** (`PlacesPage.jsx` — filtro client-side sobre `GET /places`). O backend entrega a lista completa unificada (locais de moradores + Google sync), e a UI aplica busca por nome e chips de categoria (Cachoeira, Restaurante, Pousada).
...
| RF06 | Busca por nome | Frontend `PlacesPage` — filtro sobre lista |
| RF06 | Filtro por categoria | Frontend chips + `category` state |
